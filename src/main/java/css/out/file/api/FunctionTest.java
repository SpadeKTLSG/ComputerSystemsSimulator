...
public class FunctionTest {
    ...
    public static void handleCommon(String path, String allName, FileDirTYPE type) { //Controller
        //? 通用处理
        //? 1. 鉴权
        ...
        notifyProcessSyS(mkObjectfrFront(path, allName, type));
    }
    ...
    public static void notifyProcessSyS(Object object) {
        log.info("正在通知进程系统创建进程...");
        //封装对象
        if (object instanceof file temp_file) {
            //? 文件

            //? 通知进程系统创建进程, 传递文件路径
        } else if (object instanceof dir temp_dir) {
            //? 文件夹
            //? 通知进程系统创建进程, 传递文件夹路径
        } else {
            log.error("传递的对象不是文件或文件夹");
        }
    }
